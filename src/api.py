...
# Supprimez ces imports dupliqués à la fin :
# from pydantic import BaseModel  # <-- ENLEVER CETTE LIGNE
...
class PredictionInput(BaseModel):
    ...
    class Config:
        json_schema_extra = {"example": {"ca_total": 15000.0, "nb_commandes": 320}}
...
# Nouveau schéma pour la prédiction avec alerte
class PredictWithAlertRequest(BaseModel):
    ca_total: float
    nb_commandes: int
    seuil_alerte: float = 10000  # Seuil par défaut
...
def log_prediction_to_csv(
    ca_total: float, nb_commandes: int, prediction: float, model_version: str
):
    """
    ...
    with open(PREDICTION_LOG_PATH, mode="a", newline="", encoding="utf-8") as f:
        ...
        if not file_exists:
            writer.writerow(
                ["timestamp", "ca_total", "nb_commandes", "prediction", "model_version"]
            )
...
async def root():
    ...
    return {
        ...
        "endpoints": {
            ...
            "POST /predict-with-alert": "Prédiction avec alerte",
...
@app.post("/predict-with-alert")
def predict_with_alert(data: PredictWithAlertRequest):
    """
    Prédit le CA du jour suivant et déclenche une alerte si la prédiction est en dessous d'un seuil.
    """
    try:
        if model is None:
            raise HTTPException(status_code=503, detail="Modèle non chargé")

        features = np.array([[data.ca_total, data.nb_commandes]])
        ...
        alerte = prediction < data.seuil_alerte

        return {
            "prediction_ca_jour_suivant": float(prediction),
            "seuil_alerte": data.seuil_alerte,
            "alerte_declenchee": alerte,
            "message": (
                "⚠️ Attention, la prédiction est en dessous du seuil critique !"
                if alerte
                else "✅ Prédiction conforme aux attentes."
            ),
            "timestamp": datetime.now().isoformat(),
        }
    except Exception as e:
        logger.error(f"Erreur dans predict-with-alert: {e}")
        raise HTTPException(status_code=500, detail=f"Erreur interne: {str(e)}")
...
async def model_info():
    ...
    try:
        info = {
            ...
            "n_estimators": (
                model.n_estimators if hasattr(model, "n_estimators") else None
            ),
